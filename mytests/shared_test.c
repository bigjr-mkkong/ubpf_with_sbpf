...
#include "../vm/ubpf_extfunc.h"

#include <time.h>
#include <sys/syscall.h>

#define __NR_SBPF_VERIFIER  451

/*
clang -o shared_test.o shared_test.c /usr/sbin/libubpf.so ../../linuxdev/linux/arch/x86/entry/vdso/vdso64.so
export LD_LIBRARY_PATH=../vm/libubpf.so
*/

extern int test_ubpf_load_elf(const char *code_path, const char *mem_path, double *time_comp, uint64_t *bpf_retval);

clock_t t_start, t_end;

#ifdef LOADER_IN_THIS_FILE
static void *readfile(const char *path, size_t maxlen, size_t *len);
static void register_functions(struct ubpf_vm *vm);

int vm_ret_val = 0;

int test_ubpf_load_elf(const char *code_path, const char *mem_path){
    size_t code_len;
...
int main(int argc, char ** argv){
    char codepath[] = "./test.o";
    char mempath[] = "./mem_file";
    
    uint64_t batched_times = atoi(argv[1]);
    int fd = open(mempath, O_RDWR | O_CREAT | O_TRUNC);
    write(fd, &batched_times, sizeof(unsigned long));
    close(fd);

    double time_comp = 0.0;
    uint64_t bpf_retval;
    int ret = test_ubpf_load_elf(codepath, mempath, &time_comp, &bpf_retval);
    if(ret){
    	printf("Program failed to load, abort\n");
	    exit(0);
    }

    printf("BPF program return value: %ld\n", bpf_retval);
    printf("Time spend for this bpf program(usec): %lf\n",time_comp);
    return (int)time_comp;
}
